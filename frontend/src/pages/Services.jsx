        ...
        <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetDialogState(); }}>
          ...
          <DialogContent className="max-w-lg" data-testid="service-dialog">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 mt-4">
              ...
              <div>
                <Label>{t("services.warrantyStatus") || "Warranty Status"} *</Label>
                <Select
                ...
                >
                  <SelectTrigger className="mt-1" data-testid="select-warranty">
                    <SelectValue placeholder={t("services.selectWarrantyStatus") || "Select warranty status"} />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                >
                  {t("services.saveRecord") || "Save Record"}
                </Button>
              </div>
            </form>
            )}
                  ...
                  {inServiceIssues.map((issue) => {
                    ...
                    return (
                      <InServiceIssueCard
                        key={issue.id}
                        issue={issue}
                        linkedServiceIssue={linkedServiceIssue}
                        products={products}
                        onComplete={(linkedIssue) => {
                          setSelectedIssue(linkedIssue);
                          setResolveDialogOpen(true);
                        }}
                        onViewTrack={(issueId) => fetchIssueTrack(issueId)}
                        showContactDetails={true}
                      />
                    );
                  })}
